...
//	Genome* genoTemp1 = new Genome();
	...
	initializePop(genoA, genoB, id, rank, size, chromo_len, fastaSeq, opStream);
...
//				opStream << "rank " << rank << " num_cross " << num_crossover << " fill_rest_start_index " << fill_rest_start_index << endl;
				...
				doCrossoverWithAssociatedMemory(genoA, tempRet, start, num_crossover, chromo_len, id, rank, size, generations, fastaSeq, opStream);
			...
			//	fillRest(genoA[0], tempRet, fill_rest_start_index, end, id, rank, size, generations, chromo_len, fastaSeq, opStream);
				fillRest(genoA, tempRet, fill_rest_start_index, start, end, id, rank, size, generations, chromo_len, fastaSeq, opStream);


				opStream << "finished fillRest and starting doMutation" << endl;
				
				doMutation(tempRet, start, end, chunk_width, id, rank, size, generations, chromo_len, fastaSeq, opStream);
...
//			opStream << "rank " << rank << " num_cross " << num_crossover << " fill_rest_start_index " << fill_rest_start_index << endl;
			...
			doCrossoverWithAssociatedMemory(genoA, genoB, start, num_crossover, chromo_len, id, rank, size, generations, fastaSeq, opStream);
			opStream << "finished crossover and starting fillRest" << endl;
		//	fillRest(genoA[0], genoB, fill_rest_start_index, end, id, rank, size, generations, chromo_len, fastaSeq, opStream);
			fillRest(genoA, genoB, fill_rest_start_index, start, end, id, rank, size, generations, chromo_len, fastaSeq, opStream);
			opStream << "finished fillRest and starting doMutation" << endl;
			doMutation(genoB, start, end, chunk_width, id, rank, size, generations, chromo_len, fastaSeq, opStream);
			opStream << "finished doMutation" << endl;
...
//			}
		...
		doCrossoverWithAssociatedMemory(genoA, genoB, elitist_index, num_crossover, chromo_len, id, rank, size, generations, fastaSeq);
...
void GA::createPDBFromChromosome(Genome &geno, string id, int rank, int generation, int chromo_index, string fastaSeq, int chromo_len){
...
//			cout << threeLetterAA << "\n";
	...
	string scwrlCmd = "/home/amishra2/scwrl4/Scwrl4 -i " + scwrlInputPdb + " -o " + scwrlOutputPdb;
...
void GA::createPDBFromChromosomeAfterEachGeneration(Genome &geno, string id, int rank, int generation, int chromo_index, string fastaSeq, int chromo_len){
	// create a pdb file with backbone atoms
...
void GA::calcFitness(Genome &geno, string id, int rank, int generation, int chromo_index, int chromo_len, string fastaSeq, ofstream& opStream){
	...
	createPDBFromChromosome(geno, id, rank, generation, chromo_index, fastaSeq, chromo_len);
...
void GA::doMutation(Genome *genoB, int start, int end, int chunk_width, string id, int rank, int size, int generation, int chromo_len, string fastaSeq, ofstream& opStream){
	...
	//	Genome *temp = new Genome();
		...
		bool isClash = doMutationDuringInitilization(genoB[chromo_index_to_mutate], temp, chromo_len, rank, size, fastaSeq, opStream);
//		opStream << "isClash mutation " << isClash << endl;
		while(isClash == true){
//			opStream << "doMutationDuringInit called inside mutation " << endl;
			isClash = doMutationDuringInitilization(genoB[chromo_index_to_mutate], temp, chromo_len, rank, size, fastaSeq, opStream);
//			opStream << "isClash mutation inside while loop " << isClash << endl;
		}
		
		calcFitness(temp, id, rank, generation, chromo_index_to_mutate, chromo_len, fastaSeq, opStream);
...
void GA::fillRest(Genome &genoI, Genome *tempRet, int fill_rest_start_index, int end, string id, int rank, int size, int generation, int chromo_len, string fastaSeq, ofstream& opStream){
	...
	for (int i = fill_rest_start_index; i <= end; i++){
		...
		bool isClash = doMutationDuringInitilization(genoI, tempRet[i], chromo_len, rank, size, fastaSeq, opStream);
//		opStream << "isClash fillRest " << isClash << endl;
		while(isClash == true){
//			opStream << "doMutationDuringInit called insided fillRest " << endl;
			isClash = doMutationDuringInitilization(genoI, tempRet[i], chromo_len, rank, size, fastaSeq, opStream);
//			opStream << "isClash fillRest inside while loop " << isClash << endl;
			
		}
		calcFitness(tempRet[i], id, rank, generation, i, chromo_len, fastaSeq, opStream);
...
void GA::fillRest(Genome *genoA, Genome *tempRet, int fill_rest_start_index, int start, int end, string id, int rank, int size, int generation, int chromo_len, string fastaSeq, ofstream& opStream){
	...
	for (int i = fill_rest_start_index; i <= end; i++){
		...
		bool isClash = doMutationDuringInitilization(genoA[rand], tempRet[i], chromo_len, rank, size, fastaSeq, opStream);
		opStream << "isClash fillRest " << isClash << endl;
		while (isClash == true){
			rand = getRandomBetweenTwoNumbers(0, pop_size, rank, size);
			opStream << "rand " << rand << endl;
			opStream << "doMutationDuringInit called insided fillRest " << endl;
			isClash = doMutationDuringInitilization(genoA[rand], tempRet[i], chromo_len, rank, size, fastaSeq, opStream);
			opStream << "isClash fillRest inside while loop " << isClash << endl;
...
bool GA::checkAndCorrectSSAtCrossoverPoint(Genome &genoI, Genome &newTempGenome, vector<int> crossoverPositions1, string aa, int position, int chromo_len, string id, int rank, int generation, int chromo_index, int size, string fastaSeq, ofstream& opStream){
...
//	opStream << "psi after rot " << psi_after_rot << endl;
	...
	if (hasClash == false){
		...
		calcFitness(newTempGenome, id, rank, generation, chromo_index, chromo_len, fastaSeq, opStream);
		calcFitness(tempGeno2, id, rank, generation, chromo_index, chromo_len, fastaSeq, opStream);
			...
			while (clashCutoff > 0){
				...
				if (!localClash){
					...
					if (!localClash){
						...
						calcFitness(tempGeno3, id, rank, generation, chromo_index, chromo_len, fastaSeq, opStream);
...
void GA::doCrossoverWithAssociatedMemory(Genome *genoA, Genome *genoB, int start, int num_crossover, int chromo_len, string id, int rank, int size, int generation, string fastaSeq, ofstream& opStream){

	double total_fitness = 0;
	double rand1;
	double rand2;
	int first_selection_index;
	int second_selection_index;
	int crossover_point;
...
//		opStream << "second selection index =========================================== " << second_index << endl;
	//	cout.flush();
		...
		int point = getRandomBetweenTwoNumbers(2, crossoverPositions1.size()-2, rank, size);
...
//		memcpy(genoB[newPopLoc + 2], genoA[second_index], sizeof(genoA[second_index]));
	...
	//	cout.flush();
		...
		int clash = checkAndCorrectSSAtCrossoverPoint(genoA[first_index], tempNewPopCand1, crossoverPositions1, aa, point, chromo_len, id, rank, generation, 200 + newPopLoc + 1, size, fastaSeq, opStream);
		...
		clash = checkAndCorrectSSAtCrossoverPoint(genoA[first_index], tempNewPopCand2, crossoverPositions1, aa, point, chromo_len, id, rank, generation, 200 + newPopLoc + 2, size, fastaSeq, opStream);
		...
		calcFitness(tempNewPopCand1, id, rank, generation, 200+newPopLoc+1, chromo_len, fastaSeq, opStream);
	...
	//	cout << endl;
		...
		calcFitness(tempNewPopCand2, id, rank, generation, 200+newPopLoc+2, chromo_len, fastaSeq, opStream);
	...
	//	cout << endl;
		...
		clash = checkAndCorrectSSAtCrossoverPoint(genoA[first_index], tempNewPopCand3, crossoverPositions1, aa, point, chromo_len, id, rank, generation, 200 + newPopLoc + 3, size, fastaSeq, opStream);
		...
		clash = checkAndCorrectSSAtCrossoverPoint(genoA[first_index], tempNewPopCand4, crossoverPositions1, aa, point, chromo_len, id, rank, generation, 200 + newPopLoc + 4, size, fastaSeq, opStream);
		...
		// now calculate the fitness
		calcFitness(tempNewPopCand3, id, rank, generation, 200+newPopLoc+3, chromo_len, fastaSeq, opStream);
	...
	//	cout << endl;
		...
		calcFitness(tempNewPopCand4, id, rank, generation, 200+newPopLoc+4, chromo_len, fastaSeq, opStream);
...
void GA::initializePop(Genome* genoA, Genome* genoB, string id, int rank, int size, int chromo_len, string fastaSeq, ofstream& opStream){
...
//			opStream << pdbFilePath << endl;
	...
	for(int i = 0; i < pdbFilesCounter; i++){
		...
		calcFitness(genoA[i], id, rank, generation, i, chromo_len, fastaSeq, opStream);
		firstRun = "F";
	...
	while(copyPdbFilesCounter < pdbFilesCounter){
		...
		for (int i = pdbFilesCounter; i < proportion+pdbFilesCounter; i++){
			...
			bool isClash = doMutationDuringInitilization(genoA[copyPdbFilesCounter], genoA[fillIndex], chromo_len, rank, size, fastaSeq, opStream);
			
//			opStream << "first call isClash " << isClash << endl;
			while(isClash == true){
//				opStream << "inside while loop ... initializePop" << endl;
				isClash = doMutationDuringInitilization(genoA[copyPdbFilesCounter], genoA[fillIndex], chromo_len, rank, size, fastaSeq, opStream);
//				opStream << "while loop call isClash " << isClash << endl;
			}
			calcFitness(genoA[fillIndex], id, rank, generation, fillIndex, chromo_len, fastaSeq, opStream);
	...
	for(int i = fillIndex; i < pop_size; i++){
		...
		bool isClash = doMutationDuringInitilization(genoA[lowestFitnessIndex], genoA[i], chromo_len, rank, size, fastaSeq, opStream);
			
//			opStream << "first call isClash " << isClash << endl;
			while(isClash == true){
//				opStream << "inside while loop ... initializePop" << endl;
				isClash = doMutationDuringInitilization(genoA[lowestFitnessIndex], genoA[i], chromo_len, rank, size, fastaSeq, opStream);
//				opStream << "while loop call isClash " << isClash << endl;
			}
			calcFitness(genoA[i], id, rank, generation, i, chromo_len, fastaSeq, opStream);
...
string GA::getSsFromPhiAndPsi(string aaType, double phi, double psi, ofstream& opStream){
	...
	int psiIndex = 36 - ceil((psi+180)/10);
	if(psiIndex == 36){
...
//	opStream << "chromo_len " << chromo_len << " mut_typ " << mut_typ << " mut_ind " << mut_ind << " old_ag " << old_ag << " aa " << aa << " rank " << rank << " size " << size << endl;
	...
	if(mut_typ == 1){
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ala.beta_phi.at(rw_ind) - 10, ala.beta_phi.at(rw_ind), rank, size);
		//	opStream << "new_phi_ag " << new_phi_ag << endl;
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(cys.beta_phi.at(rw_ind) - 10, cys.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(asp.beta_phi.at(rw_ind) - 10, asp.beta_phi.at(rw_ind), rank, size);
		//	opStream << "new_phi_ag " << new_phi_ag << endl;
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(glu.beta_phi.at(rw_ind) - 10, glu.beta_phi.at(rw_ind), rank, size);	
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(phe.beta_phi.at(rw_ind) - 10, phe.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(gly.beta_phi.at(rw_ind) - 10, gly.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(his.beta_phi.at(rw_ind) - 10, his.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ile.beta_phi.at(rw_ind) - 10, ile.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(lys.beta_phi.at(rw_ind) - 10, lys.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(leu.beta_phi.at(rw_ind) - 10, leu.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(met.beta_phi.at(rw_ind) - 10, met.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(asn.beta_phi.at(rw_ind) - 10, asn.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(pro.beta_phi.at(rw_ind) - 10, pro.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(gln.beta_phi.at(rw_ind) - 10, gln.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(arg.beta_phi.at(rw_ind) - 10, arg.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ser.beta_phi.at(rw_ind) - 10, ser.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(thr.beta_phi.at(rw_ind) - 10, thr.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(val.beta_phi.at(rw_ind) - 10, val.beta_phi.at(rw_ind), rank, size);
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(trp.beta_phi.at(rw_ind) - 10, trp.beta_phi.at(rw_ind), rank, size);
		//	opStream << "new_phi_ag " << new_phi_ag << endl;
		...
		//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(tyr.beta_phi.at(rw_ind) - 10, tyr.beta_phi.at(rw_ind), rank, size);
		//	opStream << "new_phi_ag " << new_phi_ag << endl;
...
//		opStream << "hasClash .. updating phi angle with best beta " << hasClash << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ala.beta_psi.at(rw_ind) - 3, ala.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ala.beta_psi.at(rw_ind), ala.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(cys.beta_psi.at(rw_ind) - 3, cys.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(cys.beta_psi.at(rw_ind), cys.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(asp.beta_psi.at(rw_ind) - 3, asp.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(asp.beta_psi.at(rw_ind), asp.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "GLU"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(glu.beta_psi.at(rw_ind) - 3, glu.beta_psi.at(rw_ind), rank, size);	
			new_psi_ag = getRandomBetweenTwoNumbers(glu.beta_psi.at(rw_ind), glu.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "PHE"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(phe.beta_psi.at(rw_ind) - 3, phe.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(phe.beta_psi.at(rw_ind), phe.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "GLY"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(gly.beta_psi.at(rw_ind) - 3, gly.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(gly.beta_psi.at(rw_ind), gly.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "HIS"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(his.beta_psi.at(rw_ind) - 3, his.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(his.beta_psi.at(rw_ind), his.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "ILE"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ile.beta_psi.at(rw_ind) - 3, ile.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ile.beta_psi.at(rw_ind), ile.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "LYS"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(lys.beta_psi.at(rw_ind) - 3, lys.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(lys.beta_psi.at(rw_ind), lys.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "LEU"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(leu.beta_psi.at(rw_ind) - 3, leu.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(leu.beta_psi.at(rw_ind), leu.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "MET"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(met.beta_psi.at(rw_ind) - 3, met.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(met.beta_psi.at(rw_ind), met.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "ASN"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(asn.beta_psi.at(rw_ind) - 3, asn.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(asn.beta_psi.at(rw_ind), asn.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "PRO"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(pro.beta_psi.at(rw_ind) - 3, pro.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(pro.beta_psi.at(rw_ind), pro.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "GLN"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(gln.beta_psi.at(rw_ind) - 3, gln.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(gln.beta_psi.at(rw_ind), gln.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "ARG"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(arg.beta_psi.at(rw_ind) - 3, arg.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(arg.beta_psi.at(rw_ind), arg.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "SER"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ser.beta_psi.at(rw_ind) - 3, ser.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ser.beta_psi.at(rw_ind), ser.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "THR"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(thr.beta_psi.at(rw_ind) - 3, thr.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(thr.beta_psi.at(rw_ind), thr.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "VAL"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(val.beta_psi.at(rw_ind) - 3, val.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(val.beta_psi.at(rw_ind), val.beta_psi.at(rw_ind) + 10, rank, size);
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(trp.beta_psi.at(rw_ind) - 3, trp.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(trp.beta_psi.at(rw_ind), trp.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}else if (aa == "TYR"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(tyr.beta_psi.at(rw_ind) - 3, tyr.beta_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(tyr.beta_psi.at(rw_ind), tyr.beta_psi.at(rw_ind) + 10, rank, size);
		//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
...
bool GA::updateChromosomeWithBestHelixPhiPsi(Genome &tempGeno1, int chromo_len, int mut_typ, int mut_ind, double old_ag, string aa, int rank, int size, ofstream& opStream){
	...
	if (mut_typ == 1){
		...
		if (aa == "ALA"){
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ala.helix_phi.at(rw_ind) - 10, ala.helix_phi.at(rw_ind), rank, size);
			//	opStream << "new_phi_ag " << new_phi_ag << endl;
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(cys.helix_phi.at(rw_ind) - 10, cys.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(asp.helix_phi.at(rw_ind) - 10, asp.helix_phi.at(rw_ind), rank, size);
			//	opStream << "new_phi_ag " << new_phi_ag << endl;
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(glu.helix_phi.at(rw_ind) - 10, glu.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(phe.helix_phi.at(rw_ind) - 10, phe.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(gly.helix_phi.at(rw_ind) - 10, gly.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(his.helix_phi.at(rw_ind) - 10, his.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ile.helix_phi.at(rw_ind) - 10, ile.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(lys.helix_phi.at(rw_ind) - 10, lys.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(leu.helix_phi.at(rw_ind) - 10, leu.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(met.helix_phi.at(rw_ind) - 10, met.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(asn.helix_phi.at(rw_ind) - 10, asn.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(pro.helix_phi.at(rw_ind) - 10, pro.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(gln.helix_phi.at(rw_ind) - 10, gln.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(arg.helix_phi.at(rw_ind) - 10, arg.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(ser.helix_phi.at(rw_ind) - 10, ser.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(thr.helix_phi.at(rw_ind) - 10, thr.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(val.helix_phi.at(rw_ind) - 10, val.helix_phi.at(rw_ind), rank, size);
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(trp.helix_phi.at(rw_ind) - 10, trp.helix_phi.at(rw_ind), rank, size);
			//	opStream << "new_phi_ag " << new_phi_ag << endl;
			...
			//	opStream << "rw_ind " << rw_ind << endl;
			new_phi_ag = getRandomBetweenTwoNumbers(tyr.helix_phi.at(rw_ind) - 10, tyr.helix_phi.at(rw_ind), rank, size);
			//	opStream << "new_phi_ag " << new_phi_ag << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ala.helix_psi.at(rw_ind) - 3, ala.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ala.helix_psi.at(rw_ind), ala.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(cys.helix_psi.at(rw_ind) - 3, cys.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(cys.helix_psi.at(rw_ind), cys.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(asp.helix_psi.at(rw_ind) - 3, asp.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(asp.helix_psi.at(rw_ind), asp.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "GLU"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(glu.helix_psi.at(rw_ind) - 3, glu.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(glu.helix_psi.at(rw_ind), glu.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "PHE"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(phe.helix_psi.at(rw_ind) - 3, phe.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(phe.helix_psi.at(rw_ind), phe.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "GLY"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(gly.helix_psi.at(rw_ind) - 3, gly.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(gly.helix_psi.at(rw_ind), gly.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "HIS"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(his.helix_psi.at(rw_ind) - 3, his.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(his.helix_psi.at(rw_ind), his.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "ILE"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ile.helix_psi.at(rw_ind) - 3, ile.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ile.helix_psi.at(rw_ind), ile.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "LYS"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(lys.helix_psi.at(rw_ind) - 3, lys.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(lys.helix_psi.at(rw_ind), lys.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "LEU"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(leu.helix_psi.at(rw_ind) - 3, leu.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(leu.helix_psi.at(rw_ind), leu.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "MET"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(met.helix_psi.at(rw_ind) - 3, met.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(met.helix_psi.at(rw_ind), met.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "ASN"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(asn.helix_psi.at(rw_ind) - 3, asn.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(asn.helix_psi.at(rw_ind), asn.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "PRO"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(pro.helix_psi.at(rw_ind) - 3, pro.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(pro.helix_psi.at(rw_ind), pro.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "GLN"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(gln.helix_psi.at(rw_ind) - 3, gln.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(gln.helix_psi.at(rw_ind), gln.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "ARG"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(arg.helix_psi.at(rw_ind) - 3, arg.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(arg.helix_psi.at(rw_ind), arg.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "SER"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(ser.helix_psi.at(rw_ind) - 3, ser.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(ser.helix_psi.at(rw_ind), ser.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "THR"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(thr.helix_psi.at(rw_ind) - 3, thr.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(thr.helix_psi.at(rw_ind), thr.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
		else if (aa == "VAL"){
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(val.helix_psi.at(rw_ind) - 3, val.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(val.helix_psi.at(rw_ind), val.helix_psi.at(rw_ind) + 10, rank, size);
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(trp.helix_psi.at(rw_ind) - 3, trp.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(trp.helix_psi.at(rw_ind), trp.helix_psi.at(rw_ind) + 10, rank, size);
		...
		//	new_psi_ag = getRandomBetweenTwoNumbers(tyr.helix_psi.at(rw_ind) - 3, tyr.helix_psi.at(rw_ind), rank, size);
			new_psi_ag = getRandomBetweenTwoNumbers(tyr.helix_psi.at(rw_ind), tyr.helix_psi.at(rw_ind) + 10, rank, size);
			//	opStream << "new_psi_ag " << new_psi_ag << endl;
		}
...
bool GA::doMutationDuringInitilization(Genome &genoI, Genome &genoO, int chromo_len, int rank, int size, string fastaSeq, ofstream& opStream){
...
//		opStream << "two_below_ss_type " << two_below_ss_type << endl;
		...
		if (aa == "ALA"){
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ala.zone1_phi.at(rw_index) - 10, ala.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(ala.zone2, ala.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ala.zone2_phi.at(rw_index) - 10, ala.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(ala.zone3, ala.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ala.zone3_phi.at(rw_index) - 10, ala.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(cys.zone1_phi.at(rw_index) - 10, cys.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(cys.zone2, cys.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(cys.zone2_phi.at(rw_index) - 10, cys.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){
				
				rw_index = performRouletWheel(cys.zone3, cys.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(cys.zone3_phi.at(rw_index) - 10, cys.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asp.zone1_phi.at(rw_index) - 10, asp.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(asp.zone2, asp.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asp.zone2_phi.at(rw_index) - 10, asp.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(asp.zone3, asp.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asp.zone3_phi.at(rw_index) - 10, asp.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(glu.zone1_phi.at(rw_index) - 10, glu.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(glu.zone2, glu.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(glu.zone2_phi.at(rw_index) - 10, glu.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(glu.zone3, glu.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(glu.zone3_phi.at(rw_index) - 10, glu.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(phe.zone1_phi.at(rw_index) - 10, phe.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(phe.zone2, phe.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(phe.zone2_phi.at(rw_index) - 10, phe.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(phe.zone3, phe.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(phe.zone3_phi.at(rw_index) - 10, phe.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone1_phi.at(rw_index) - 10, gly.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(gly.zone2, gly.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone2_phi.at(rw_index) - 10, gly.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(gly.zone3, gly.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone3_phi.at(rw_index) - 10, gly.zone3_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 4){

				rw_index = performRouletWheel(gly.zone4, gly.zone4_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone4_phi.at(rw_index) - 10, gly.zone4_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 5){

				rw_index = performRouletWheel(gly.zone5, gly.zone5_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone5_phi.at(rw_index) - 10, gly.zone5_phi.at(rw_index), rank, size);
				
			}else if (selected_zone == 6){

				rw_index = performRouletWheel(gly.zone6, gly.zone6_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gly.zone6_phi.at(rw_index) - 10, gly.zone6_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(his.zone1_phi.at(rw_index) - 10, his.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(his.zone2, his.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(his.zone2_phi.at(rw_index) - 10, his.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(his.zone3, his.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(his.zone3_phi.at(rw_index) - 10, his.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ile.zone1_phi.at(rw_index) - 10, ile.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(ile.zone2, ile.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ile.zone2_phi.at(rw_index) - 10, ile.zone2_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(lys.zone1_phi.at(rw_index) - 10, lys.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(lys.zone2, lys.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(lys.zone2_phi.at(rw_index) - 10, lys.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(lys.zone3, lys.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(lys.zone3_phi.at(rw_index) - 10, lys.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(leu.zone1_phi.at(rw_index) - 10, leu.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(leu.zone2, leu.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(leu.zone2_phi.at(rw_index) - 10, leu.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(leu.zone3, leu.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(leu.zone3_phi.at(rw_index) - 10, leu.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(met.zone1_phi.at(rw_index) - 10, met.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(met.zone2, met.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(met.zone2_phi.at(rw_index) - 10, met.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(met.zone3, met.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(met.zone3_phi.at(rw_index) - 10, met.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asn.zone1_phi.at(rw_index) - 10, asn.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(asn.zone2, asn.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asn.zone2_phi.at(rw_index) - 10, asn.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(asn.zone3, asn.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(asn.zone3_phi.at(rw_index) - 10, asn.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(pro.zone1_phi.at(rw_index) - 10, pro.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(pro.zone2, pro.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(pro.zone2_phi.at(rw_index) - 10, pro.zone2_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gln.zone1_phi.at(rw_index) - 10, gln.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(gln.zone2, gln.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gln.zone2_phi.at(rw_index) - 10, gln.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(gln.zone3, gln.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(gln.zone3_phi.at(rw_index) - 10, gln.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(arg.zone1_phi.at(rw_index) - 10, arg.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(arg.zone2, arg.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(arg.zone2_phi.at(rw_index) - 10, arg.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(arg.zone3, arg.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(arg.zone3_phi.at(rw_index) - 10, arg.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ser.zone1_phi.at(rw_index) - 10, ser.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(ser.zone2, ser.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ser.zone2_phi.at(rw_index) - 10, ser.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(ser.zone3, ser.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(ser.zone3_phi.at(rw_index) - 10, ser.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(thr.zone1_phi.at(rw_index) - 10, thr.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(thr.zone2, thr.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(thr.zone2_phi.at(rw_index) - 10, thr.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(thr.zone3, thr.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(thr.zone3_phi.at(rw_index) - 10, thr.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(val.zone1_phi.at(rw_index) - 10, val.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(val.zone2, val.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(val.zone2_phi.at(rw_index) - 10, val.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(val.zone3, val.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(val.zone3_phi.at(rw_index) - 10, val.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(trp.zone1_phi.at(rw_index) - 10, trp.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(trp.zone2, trp.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(trp.zone2_phi.at(rw_index) - 10, trp.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(trp.zone3, trp.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(trp.zone3_phi.at(rw_index) - 10, trp.zone3_phi.at(rw_index), rank, size);
			...
			if (selected_zone == 1){
				...
				new_phi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone1_phi.at(rw_index) - 10, tyr.zone1_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(tyr.zone2, tyr.zone2_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone2_phi.at(rw_index) - 10, tyr.zone2_phi.at(rw_index), rank, size);
				
			}
			else if (selected_zone == 3){

				rw_index = performRouletWheel(tyr.zone3, tyr.zone3_total_freq, rank, size);
				new_phi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone3_phi.at(rw_index) - 10, tyr.zone3_phi.at(rw_index), rank, size);
...
//				opStream << "hasClashPlus inside mut during init ... after removing steric clash phi " << hasClashPlus << endl;
		...
		mut_pos = getRandomBetweenTwoNumbers(2, psiAgMutPos.size()-2, rank, size);
...
//		opStream << "two_below_ss_type " << two_below_ss_type << endl;
		...
		if (aa == "ALA"){
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone1_psi.at(rw_index) - 3, ala.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone1_psi.at(rw_index), ala.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone2_psi.at(rw_index) - 3, ala.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone2_psi.at(rw_index), ala.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone3_psi.at(rw_index) - 3, ala.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ala.zone3_psi.at(rw_index), ala.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone1_psi.at(rw_index) - 3, cys.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone1_psi.at(rw_index), cys.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone2_psi.at(rw_index) - 3, cys.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone2_psi.at(rw_index), cys.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone3_psi.at(rw_index) - 3, cys.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(cys.zone3_psi.at(rw_index), cys.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone1_psi.at(rw_index) - 3, asp.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone1_psi.at(rw_index), asp.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone2_psi.at(rw_index) - 3, asp.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone2_psi.at(rw_index), asp.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone3_psi.at(rw_index) - 3, asp.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asp.zone3_psi.at(rw_index), asp.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone1_psi.at(rw_index) - 3, glu.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone1_psi.at(rw_index), glu.zone1_psi.at(rw_index) + 10, rank, size);
			}
			else if (selected_zone == 2){

				rw_index = performRouletWheel(glu.zone2, glu.zone2_total_freq, rank, size);
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone2_psi.at(rw_index) - 3, glu.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone2_psi.at(rw_index), glu.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone3_psi.at(rw_index) - 3, glu.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(glu.zone3_psi.at(rw_index), glu.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone1_psi.at(rw_index) - 3, phe.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone1_psi.at(rw_index), phe.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone2_psi.at(rw_index) - 3, phe.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone2_psi.at(rw_index), phe.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone3_psi.at(rw_index) - 3, phe.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(phe.zone3_psi.at(rw_index), phe.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone1_psi.at(rw_index) - 3, gly.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone1_psi.at(rw_index), gly.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone2_psi.at(rw_index) - 3, gly.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone2_psi.at(rw_index), gly.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone3_psi.at(rw_index) - 3, gly.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone3_psi.at(rw_index), gly.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone4_psi.at(rw_index) - 3, gly.zone4_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone4_psi.at(rw_index), gly.zone4_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone5_psi.at(rw_index) - 3, gly.zone5_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone5_psi.at(rw_index), gly.zone5_psi.at(rw_index) + 10, rank, size);
				
			}else if (selected_zone == 6){

				rw_index = performRouletWheel(gly.zone6, gly.zone6_total_freq, rank, size);
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone6_psi.at(rw_index) - 3, gly.zone6_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gly.zone6_psi.at(rw_index), gly.zone6_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone1_psi.at(rw_index) - 3, his.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone1_psi.at(rw_index), his.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone2_psi.at(rw_index) - 3, his.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone2_psi.at(rw_index), his.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone3_psi.at(rw_index) - 3, his.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(his.zone3_psi.at(rw_index), his.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(ile.zone1_psi.at(rw_index) - 3, ile.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ile.zone1_psi.at(rw_index), ile.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(ile.zone2_psi.at(rw_index) - 3, ile.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ile.zone2_psi.at(rw_index), ile.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone1_psi.at(rw_index) - 3, lys.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone1_psi.at(rw_index), lys.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone2_psi.at(rw_index) - 3, lys.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone2_psi.at(rw_index), lys.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone3_psi.at(rw_index) - 3, lys.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(lys.zone3_psi.at(rw_index), lys.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone1_psi.at(rw_index) - 3, leu.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone1_psi.at(rw_index), leu.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone2_psi.at(rw_index) - 3, leu.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone2_psi.at(rw_index), leu.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone3_psi.at(rw_index) - 3, leu.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(leu.zone3_psi.at(rw_index), leu.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone1_psi.at(rw_index) - 3, met.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone1_psi.at(rw_index), met.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone2_psi.at(rw_index) - 3, met.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone2_psi.at(rw_index), met.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone3_psi.at(rw_index) - 3, met.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(met.zone3_psi.at(rw_index), met.zone3_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone1_psi.at(rw_index) - 3, asn.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone1_psi.at(rw_index), asn.zone1_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone2_psi.at(rw_index) - 3, asn.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone2_psi.at(rw_index), asn.zone2_psi.at(rw_index) + 10, rank, size);
			...
			//	new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone3_psi.at(rw_index) - 3, asn.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(asn.zone3_psi.at(rw_index), asn.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(pro.zone1_psi.at(rw_index) - 3, pro.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(pro.zone1_psi.at(rw_index), pro.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(pro.zone2_psi.at(rw_index) - 3, pro.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(pro.zone2_psi.at(rw_index), pro.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone1_psi.at(rw_index) - 3, gln.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone1_psi.at(rw_index), gln.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone2_psi.at(rw_index) - 3, gln.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone2_psi.at(rw_index), gln.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone3_psi.at(rw_index) - 3, gln.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(gln.zone3_psi.at(rw_index), gln.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone1_psi.at(rw_index) - 3, arg.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone1_psi.at(rw_index), arg.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone2_psi.at(rw_index) - 3, arg.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone2_psi.at(rw_index), arg.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone3_psi.at(rw_index) - 3, arg.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(arg.zone3_psi.at(rw_index), arg.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone1_psi.at(rw_index) - 3, ser.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone1_psi.at(rw_index), ser.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone2_psi.at(rw_index) - 3, ser.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone2_psi.at(rw_index), ser.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone3_psi.at(rw_index) - 3, ser.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(ser.zone3_psi.at(rw_index), ser.zone3_psi.at(rw_index) + 10, rank, size);
	...
	//			new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone1_psi.at(rw_index) - 3, thr.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone1_psi.at(rw_index), thr.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone2_psi.at(rw_index) - 3, thr.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone2_psi.at(rw_index), thr.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone3_psi.at(rw_index) - 3, thr.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(thr.zone3_psi.at(rw_index), thr.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone1_psi.at(rw_index) - 3, val.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone1_psi.at(rw_index), val.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone2_psi.at(rw_index) - 3, val.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone2_psi.at(rw_index), val.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone3_psi.at(rw_index) - 3, val.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(val.zone3_psi.at(rw_index), val.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone1_psi.at(rw_index) - 3, trp.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone1_psi.at(rw_index), trp.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone2_psi.at(rw_index) - 3, trp.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone2_psi.at(rw_index), trp.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone3_psi.at(rw_index) - 3, trp.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(trp.zone3_psi.at(rw_index), trp.zone3_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone1_psi.at(rw_index) - 3, tyr.zone1_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone1_psi.at(rw_index), tyr.zone1_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone2_psi.at(rw_index) - 3, tyr.zone2_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone2_psi.at(rw_index), tyr.zone2_psi.at(rw_index) + 10, rank, size);
		...
		//		new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone3_psi.at(rw_index) - 3, tyr.zone3_psi.at(rw_index), rank, size);
				new_psi_ag_plus = getRandomBetweenTwoNumbers(tyr.zone3_psi.at(rw_index), tyr.zone3_psi.at(rw_index) + 10, rank, size);
				...
				//	int total_freq_zone4 = 0;
					...
					for (int j = 0; j <= 17; j++){	// psi loop
						...
						while (linestream >> temp){
							...
							else if (phi_counter > 16 && phi_counter <= 34){
				...
				//	phe.zone4_total_freq = total_freq_zone4;
					...
					for (int j = 0; j <= 9; j++){	// psi loop
						...
						while (linestream >> temp){
							...
							else if (phi_counter > 17 && phi_counter <= 34){
